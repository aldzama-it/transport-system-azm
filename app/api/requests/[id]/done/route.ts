...
import { finishRequest } from "@/lib/requests";
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const request = await finishRequest(id, staffId);

    return apiSuccess(request);
  } catch (error: any) {
    console.error("Error finishing request:", error);
    return apiError(error.message || "Terjadi kesalahan pada server saat menyelesaikan pengajuan", 500);
  }
}
