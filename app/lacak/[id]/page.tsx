...
import { FileText, Calendar, Building, MapPin, Car, UserCircle, AlertCircle, CheckCircle2, Clock, Ban, Phone } from "lucide-react";
...
export default function TrackingDetail() {
  ...
  const handleCancel = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!cancelReason.trim() || !selectedRequest) return;
  ...
  if (isLoading) {
    return (
      <div className="max-w-6xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <HomeTabs />
        <div className="flex justify-center py-20">Memuat detail...</div>
      </div>
    );
  }
            ...
            <div className="bg-slate-50 rounded-3xl p-6 border border-slate-100">
              ...
              <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-300 before:to-transparent">
                {selectedRequest.history.map((hist: any) => {
                  const stat = statusConfig[hist.status] || statusConfig.pending;
